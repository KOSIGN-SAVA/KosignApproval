...
public struct JLToastViewValue {
    static var FontSize: CGFloat {
        get { return UIDevice.currentDevice().userInterfaceIdiom == UIUserInterfaceIdiom.Phone ? 55 : 16 }
    }
    
    static var PortraitOffsetY: CGFloat {
        get { return UIDevice.currentDevice().userInterfaceIdiom == UIUserInterfaceIdiom.Phone ? 230 : 60 }
    }
            ...
            UIView.animateWithDuration(0.1,
                ...
                completion: { (completed: Bool) in
                    UIView.animateWithDuration(self.duration!,
                        ...
                        completion: { (completed: Bool) in
                            self.finish()
                            UIView.animateWithDuration(0.1, animations: { () in self._view!.alpha = 0 })
                    })
            })
        })
    }
